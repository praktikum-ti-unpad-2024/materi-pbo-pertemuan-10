  ...
  md.block.ruler.after(
    ...
    function mdc_block_yaml(state, startLine, endLine, silent) {
      ...
      if (!silent) {
        ...
        const data = YAML__default.load(yaml);
    ...
    tokens.forEach((token, index) => {
      if (token.type === "mdc_inline_props") {
        ...
        if (!prev.tag && prev.type === "text") {
          prev = new Token__default("mdc_inline_span", "span", 1);
          tokens.splice(index - 1, 0, prev);
          const close = new Token__default("mdc_inline_span", "span", -1);
