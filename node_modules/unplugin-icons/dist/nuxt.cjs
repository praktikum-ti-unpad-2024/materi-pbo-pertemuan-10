...
var _chunkY4PLMDSTcjs = require('./chunk-Y4PLMDST.cjs');
...
function nuxt_default(options = {}, nuxt) {
  ...
  nuxtApp.hook("webpack:config", (configs) => {
    configs.forEach((config) => {
      ...
      config.plugins.unshift(_chunkY4PLMDSTcjs.src_default.webpack(options));
    });
  });
  nuxtApp.hook("vite:extend", async (vite) => {
    vite.config.plugins = vite.config.plugins || [];
    vite.config.plugins.push(_chunkY4PLMDSTcjs.src_default.vite(options));
  });
}
