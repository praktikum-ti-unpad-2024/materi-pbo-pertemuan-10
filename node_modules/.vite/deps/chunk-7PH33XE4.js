import {
  t
} from "./chunk-237VK7HS.js";
import {
  An,
  No,
  hi,
  ie,
  ku
} from "./chunk-CC5SJ7OS.js";

// node_modules/mermaid/dist/linear-483c1a28.js
function F(n, t2) {
  return n == null || t2 == null ? NaN : n < t2 ? -1 : n > t2 ? 1 : n >= t2 ? 0 : NaN;
}
function hn(n, t2) {
  return n == null || t2 == null ? NaN : t2 < n ? -1 : t2 > n ? 1 : t2 >= n ? 0 : NaN;
}
function _(n) {
  let t2, e, r;
  n.length !== 2 ? (t2 = F, e = (u, c) => F(n(u), c), r = (u, c) => n(u) - c) : (t2 = n === F || n === hn ? n : mn, e = n, r = n);
  function i(u, c, o = 0, l = u.length) {
    if (o < l) {
      if (t2(c, c) !== 0)
        return l;
      do {
        const h = o + l >>> 1;
        e(u[h], c) < 0 ? o = h + 1 : l = h;
      } while (o < l);
    }
    return o;
  }
  function f(u, c, o = 0, l = u.length) {
    if (o < l) {
      if (t2(c, c) !== 0)
        return l;
      do {
        const h = o + l >>> 1;
        e(u[h], c) <= 0 ? o = h + 1 : l = h;
      } while (o < l);
    }
    return o;
  }
  function a(u, c, o = 0, l = u.length) {
    const h = i(u, c, o, l - 1);
    return h > o && r(u[h - 1], c) > -r(u[h], c) ? h - 1 : h;
  }
  return { left: i, center: a, right: f };
}
function mn() {
  return 0;
}
function sn(n) {
  return n === null ? NaN : +n;
}
var ln = _(F);
var dn = ln.right;
_(sn).center;
var gn = dn;
var yn = Math.sqrt(50);
var Mn = Math.sqrt(10);
var pn = Math.sqrt(2);
function R(n, t2, e) {
  const r = (t2 - n) / Math.max(0, e), i = Math.floor(Math.log10(r)), f = r / Math.pow(10, i), a = f >= yn ? 10 : f >= Mn ? 5 : f >= pn ? 2 : 1;
  let u, c, o;
  return i < 0 ? (o = Math.pow(10, -i) / a, u = Math.round(n * o), c = Math.round(t2 * o), u / o < n && ++u, c / o > t2 && --c, o = -o) : (o = Math.pow(10, i) * a, u = Math.round(n / o), c = Math.round(t2 / o), u * o < n && ++u, c * o > t2 && --c), c < u && 0.5 <= e && e < 2 ? R(n, t2, e * 2) : [u, c, o];
}
function wn(n, t2, e) {
  if (t2 = +t2, n = +n, e = +e, !(e > 0))
    return [];
  if (n === t2)
    return [n];
  const r = t2 < n, [i, f, a] = r ? R(t2, n, e) : R(n, t2, e);
  if (!(f >= i))
    return [];
  const u = f - i + 1, c = new Array(u);
  if (r)
    if (a < 0)
      for (let o = 0; o < u; ++o)
        c[o] = (f - o) / -a;
    else
      for (let o = 0; o < u; ++o)
        c[o] = (f - o) * a;
  else if (a < 0)
    for (let o = 0; o < u; ++o)
      c[o] = (i + o) / -a;
  else
    for (let o = 0; o < u; ++o)
      c[o] = (i + o) * a;
  return c;
}
function L(n, t2, e) {
  return t2 = +t2, n = +n, e = +e, R(n, t2, e)[2];
}
function Nn(n, t2, e) {
  t2 = +t2, n = +n, e = +e;
  const r = t2 < n, i = r ? L(t2, n, e) : L(n, t2, e);
  return (r ? -1 : 1) * (i < 0 ? 1 / -i : i);
}
function kn(n, t2) {
  t2 || (t2 = []);
  var e = n ? Math.min(t2.length, n.length) : 0, r = t2.slice(), i;
  return function(f) {
    for (i = 0; i < e; ++i)
      r[i] = n[i] * (1 - f) + t2[i] * f;
    return r;
  };
}
function xn(n) {
  return ArrayBuffer.isView(n) && !(n instanceof DataView);
}
function An2(n, t2) {
  var e = t2 ? t2.length : 0, r = n ? Math.min(e, n.length) : 0, i = new Array(r), f = new Array(e), a;
  for (a = 0; a < r; ++a)
    i[a] = C(n[a], t2[a]);
  for (; a < e; ++a)
    f[a] = t2[a];
  return function(u) {
    for (a = 0; a < r; ++a)
      f[a] = i[a](u);
    return f;
  };
}
function vn(n, t2) {
  var e = /* @__PURE__ */ new Date();
  return n = +n, t2 = +t2, function(r) {
    return e.setTime(n * (1 - r) + t2 * r), e;
  };
}
function Sn(n, t2) {
  var e = {}, r = {}, i;
  (n === null || typeof n != "object") && (n = {}), (t2 === null || typeof t2 != "object") && (t2 = {});
  for (i in t2)
    i in n ? e[i] = C(n[i], t2[i]) : r[i] = t2[i];
  return function(f) {
    for (i in e)
      r[i] = e[i](f);
    return r;
  };
}
function C(n, t2) {
  var e = typeof t2, r;
  return t2 == null || e === "boolean" ? An(t2) : (e === "number" ? ie : e === "string" ? (r = hi(t2)) ? (t2 = r, No) : ku : t2 instanceof hi ? No : t2 instanceof Date ? vn : xn(t2) ? kn : Array.isArray(t2) ? An2 : typeof t2.valueOf != "function" && typeof t2.toString != "function" || isNaN(t2) ? Sn : ie)(n, t2);
}
function bn(n, t2) {
  return n = +n, t2 = +t2, function(e) {
    return Math.round(n * (1 - e) + t2 * e);
  };
}
function jn(n) {
  return Math.abs(n = Math.round(n)) >= 1e21 ? n.toLocaleString("en").replace(/,/g, "") : n.toString(10);
}
function E(n, t2) {
  if ((e = (n = t2 ? n.toExponential(t2 - 1) : n.toExponential()).indexOf("e")) < 0)
    return null;
  var e, r = n.slice(0, e);
  return [
    r.length > 1 ? r[0] + r.slice(2) : r,
    +n.slice(e + 1)
  ];
}
function v(n) {
  return n = E(Math.abs(n)), n ? n[1] : NaN;
}
function Pn(n, t2) {
  return function(e, r) {
    for (var i = e.length, f = [], a = 0, u = n[0], c = 0; i > 0 && u > 0 && (c + u + 1 > r && (u = Math.max(1, r - c)), f.push(e.substring(i -= u, i + u)), !((c += u + 1) > r)); )
      u = n[a = (a + 1) % n.length];
    return f.reverse().join(t2);
  };
}
function zn(n) {
  return function(t2) {
    return t2.replace(/[0-9]/g, function(e) {
      return n[+e];
    });
  };
}
var $n = /^(?:(.)?([<>=^]))?([+\-( ])?([$#])?(0)?(\d+)?(,)?(\.\d+)?(~)?([a-z%])?$/i;
function D(n) {
  if (!(t2 = $n.exec(n)))
    throw new Error("invalid format: " + n);
  var t2;
  return new B({
    fill: t2[1],
    align: t2[2],
    sign: t2[3],
    symbol: t2[4],
    zero: t2[5],
    width: t2[6],
    comma: t2[7],
    precision: t2[8] && t2[8].slice(1),
    trim: t2[9],
    type: t2[10]
  });
}
D.prototype = B.prototype;
function B(n) {
  this.fill = n.fill === void 0 ? " " : n.fill + "", this.align = n.align === void 0 ? ">" : n.align + "", this.sign = n.sign === void 0 ? "-" : n.sign + "", this.symbol = n.symbol === void 0 ? "" : n.symbol + "", this.zero = !!n.zero, this.width = n.width === void 0 ? void 0 : +n.width, this.comma = !!n.comma, this.precision = n.precision === void 0 ? void 0 : +n.precision, this.trim = !!n.trim, this.type = n.type === void 0 ? "" : n.type + "";
}
B.prototype.toString = function() {
  return this.fill + this.align + this.sign + this.symbol + (this.zero ? "0" : "") + (this.width === void 0 ? "" : Math.max(1, this.width | 0)) + (this.comma ? "," : "") + (this.precision === void 0 ? "" : "." + Math.max(0, this.precision | 0)) + (this.trim ? "~" : "") + this.type;
};
function Fn(n) {
  n:
    for (var t2 = n.length, e = 1, r = -1, i; e < t2; ++e)
      switch (n[e]) {
        case ".":
          r = i = e;
          break;
        case "0":
          r === 0 && (r = e), i = e;
          break;
        default:
          if (!+n[e])
            break n;
          r > 0 && (r = 0);
          break;
      }
  return r > 0 ? n.slice(0, r) + n.slice(i + 1) : n;
}
var nn;
function Rn(n, t2) {
  var e = E(n, t2);
  if (!e)
    return n + "";
  var r = e[0], i = e[1], f = i - (nn = Math.max(-8, Math.min(8, Math.floor(i / 3))) * 3) + 1, a = r.length;
  return f === a ? r : f > a ? r + new Array(f - a + 1).join("0") : f > 0 ? r.slice(0, f) + "." + r.slice(f) : "0." + new Array(1 - f).join("0") + E(n, Math.max(0, t2 + f - 1))[0];
}
function Z(n, t2) {
  var e = E(n, t2);
  if (!e)
    return n + "";
  var r = e[0], i = e[1];
  return i < 0 ? "0." + new Array(-i).join("0") + r : r.length > i + 1 ? r.slice(0, i + 1) + "." + r.slice(i + 1) : r + new Array(i - r.length + 2).join("0");
}
var H = {
  "%": (n, t2) => (n * 100).toFixed(t2),
  b: (n) => Math.round(n).toString(2),
  c: (n) => n + "",
  d: jn,
  e: (n, t2) => n.toExponential(t2),
  f: (n, t2) => n.toFixed(t2),
  g: (n, t2) => n.toPrecision(t2),
  o: (n) => Math.round(n).toString(8),
  p: (n, t2) => Z(n * 100, t2),
  r: Z,
  s: Rn,
  X: (n) => Math.round(n).toString(16).toUpperCase(),
  x: (n) => Math.round(n).toString(16)
};
function J(n) {
  return n;
}
var K = Array.prototype.map;
var Q = ["y", "z", "a", "f", "p", "n", "µ", "m", "", "k", "M", "G", "T", "P", "E", "Z", "Y"];
function En(n) {
  var t2 = n.grouping === void 0 || n.thousands === void 0 ? J : Pn(K.call(n.grouping, Number), n.thousands + ""), e = n.currency === void 0 ? "" : n.currency[0] + "", r = n.currency === void 0 ? "" : n.currency[1] + "", i = n.decimal === void 0 ? "." : n.decimal + "", f = n.numerals === void 0 ? J : zn(K.call(n.numerals, String)), a = n.percent === void 0 ? "%" : n.percent + "", u = n.minus === void 0 ? "−" : n.minus + "", c = n.nan === void 0 ? "NaN" : n.nan + "";
  function o(h) {
    h = D(h);
    var s = h.fill, p = h.align, y = h.sign, S = h.symbol, k = h.zero, b = h.width, T = h.comma, w = h.precision, G = h.trim, d = h.type;
    d === "n" ? (T = true, d = "g") : H[d] || (w === void 0 && (w = 12), G = true, d = "g"), (k || s === "0" && p === "=") && (k = true, s = "0", p = "=");
    var en = S === "$" ? e : S === "#" && /[boxX]/.test(d) ? "0" + d.toLowerCase() : "", on = S === "$" ? r : /[%p]/.test(d) ? a : "", O = H[d], an = /[defgprs%]/.test(d);
    w = w === void 0 ? 6 : /[gprs]/.test(d) ? Math.max(1, Math.min(21, w)) : Math.max(0, Math.min(20, w));
    function V(m) {
      var N = en, g = on, x, X, j;
      if (d === "c")
        g = O(m) + g, m = "";
      else {
        m = +m;
        var P = m < 0 || 1 / m < 0;
        if (m = isNaN(m) ? c : O(Math.abs(m), w), G && (m = Fn(m)), P && +m == 0 && y !== "+" && (P = false), N = (P ? y === "(" ? y : u : y === "-" || y === "(" ? "" : y) + N, g = (d === "s" ? Q[8 + nn / 3] : "") + g + (P && y === "(" ? ")" : ""), an) {
          for (x = -1, X = m.length; ++x < X; )
            if (j = m.charCodeAt(x), 48 > j || j > 57) {
              g = (j === 46 ? i + m.slice(x + 1) : m.slice(x)) + g, m = m.slice(0, x);
              break;
            }
        }
      }
      T && !k && (m = t2(m, 1 / 0));
      var z = N.length + m.length + g.length, M = z < b ? new Array(b - z + 1).join(s) : "";
      switch (T && k && (m = t2(M + m, M.length ? b - g.length : 1 / 0), M = ""), p) {
        case "<":
          m = N + m + g + M;
          break;
        case "=":
          m = N + M + m + g;
          break;
        case "^":
          m = M.slice(0, z = M.length >> 1) + N + m + g + M.slice(z);
          break;
        default:
          m = M + N + m + g;
          break;
      }
      return f(m);
    }
    return V.toString = function() {
      return h + "";
    }, V;
  }
  function l(h, s) {
    var p = o((h = D(h), h.type = "f", h)), y = Math.max(-8, Math.min(8, Math.floor(v(s) / 3))) * 3, S = Math.pow(10, -y), k = Q[8 + y / 3];
    return function(b) {
      return p(S * b) + k;
    };
  }
  return {
    format: o,
    formatPrefix: l
  };
}
var $;
var tn;
var rn;
Dn({
  thousands: ",",
  grouping: [3],
  currency: ["$", ""]
});
function Dn(n) {
  return $ = En(n), tn = $.format, rn = $.formatPrefix, $;
}
function Tn(n) {
  return Math.max(0, -v(Math.abs(n)));
}
function In(n, t2) {
  return Math.max(0, Math.max(-8, Math.min(8, Math.floor(v(t2) / 3))) * 3 - v(Math.abs(n)));
}
function Ln(n, t2) {
  return n = Math.abs(n), t2 = Math.abs(t2) - n, Math.max(0, v(t2) - v(n)) + 1;
}
function qn(n) {
  return function() {
    return n;
  };
}
function Cn(n) {
  return +n;
}
var W = [0, 1];
function A(n) {
  return n;
}
function q(n, t2) {
  return (t2 -= n = +n) ? function(e) {
    return (e - n) / t2;
  } : qn(isNaN(t2) ? NaN : 0.5);
}
function Bn(n, t2) {
  var e;
  return n > t2 && (e = n, n = t2, t2 = e), function(r) {
    return Math.max(n, Math.min(t2, r));
  };
}
function Gn(n, t2, e) {
  var r = n[0], i = n[1], f = t2[0], a = t2[1];
  return i < r ? (r = q(i, r), f = e(a, f)) : (r = q(r, i), f = e(f, a)), function(u) {
    return f(r(u));
  };
}
function On(n, t2, e) {
  var r = Math.min(n.length, t2.length) - 1, i = new Array(r), f = new Array(r), a = -1;
  for (n[r] < n[0] && (n = n.slice().reverse(), t2 = t2.slice().reverse()); ++a < r; )
    i[a] = q(n[a], n[a + 1]), f[a] = e(t2[a], t2[a + 1]);
  return function(u) {
    var c = gn(n, u, 1, r) - 1;
    return f[c](i[c](u));
  };
}
function Vn(n, t2) {
  return t2.domain(n.domain()).range(n.range()).interpolate(n.interpolate()).clamp(n.clamp()).unknown(n.unknown());
}
function Xn() {
  var n = W, t2 = W, e = C, r, i, f, a = A, u, c, o;
  function l() {
    var s = Math.min(n.length, t2.length);
    return a !== A && (a = Bn(n[0], n[s - 1])), u = s > 2 ? On : Gn, c = o = null, h;
  }
  function h(s) {
    return s == null || isNaN(s = +s) ? f : (c || (c = u(n.map(r), t2, e)))(r(a(s)));
  }
  return h.invert = function(s) {
    return a(i((o || (o = u(t2, n.map(r), ie)))(s)));
  }, h.domain = function(s) {
    return arguments.length ? (n = Array.from(s, Cn), l()) : n.slice();
  }, h.range = function(s) {
    return arguments.length ? (t2 = Array.from(s), l()) : t2.slice();
  }, h.rangeRound = function(s) {
    return t2 = Array.from(s), e = bn, l();
  }, h.clamp = function(s) {
    return arguments.length ? (a = s ? true : A, l()) : a !== A;
  }, h.interpolate = function(s) {
    return arguments.length ? (e = s, l()) : e;
  }, h.unknown = function(s) {
    return arguments.length ? (f = s, h) : f;
  }, function(s, p) {
    return r = s, i = p, l();
  };
}
function Un() {
  return Xn()(A, A);
}
function Yn(n, t2, e, r) {
  var i = Nn(n, t2, e), f;
  switch (r = D(r ?? ",f"), r.type) {
    case "s": {
      var a = Math.max(Math.abs(n), Math.abs(t2));
      return r.precision == null && !isNaN(f = In(i, a)) && (r.precision = f), rn(r, a);
    }
    case "":
    case "e":
    case "g":
    case "p":
    case "r": {
      r.precision == null && !isNaN(f = Ln(i, Math.max(Math.abs(n), Math.abs(t2)))) && (r.precision = f - (r.type === "e"));
      break;
    }
    case "f":
    case "%": {
      r.precision == null && !isNaN(f = Tn(i)) && (r.precision = f - (r.type === "%") * 2);
      break;
    }
  }
  return tn(r);
}
function Zn(n) {
  var t2 = n.domain;
  return n.ticks = function(e) {
    var r = t2();
    return wn(r[0], r[r.length - 1], e ?? 10);
  }, n.tickFormat = function(e, r) {
    var i = t2();
    return Yn(i[0], i[i.length - 1], e ?? 10, r);
  }, n.nice = function(e) {
    e == null && (e = 10);
    var r = t2(), i = 0, f = r.length - 1, a = r[i], u = r[f], c, o, l = 10;
    for (u < a && (o = a, a = u, u = o, o = i, i = f, f = o); l-- > 0; ) {
      if (o = L(a, u, e), o === c)
        return r[i] = a, r[f] = u, t2(r);
      if (o > 0)
        a = Math.floor(a / o) * o, u = Math.ceil(u / o) * o;
      else if (o < 0)
        a = Math.ceil(a * o) / o, u = Math.floor(u * o) / o;
      else
        break;
      c = o;
    }
    return n;
  }, n;
}
function Hn() {
  var n = Un();
  return n.copy = function() {
    return Vn(n, Hn());
  }, t.apply(n, arguments), Zn(n);
}

export {
  _,
  Nn,
  Vn,
  Un,
  Hn
};
//# sourceMappingURL=chunk-7PH33XE4.js.map
