...
// node_modules/mermaid/dist/sankeyDiagram-3e8fe5d5.js
var tt = function() {
    ...
    parse: function(i) {
      ...
      for (var w, O, T, V, e = {}, f, l, h2, r2; ; ) {
        if (O = c[c.length - 1], this.defaultActions[O] ? T = this.defaultActions[O] : ((w === null || typeof w > "u") && (w = g()), T = x[O] && x[O][w]), typeof T > "u" || !T.length || !T[0]) {
          ...
          r2 = [];
          for (f in x[O])
            this.terminals_[f] && f > N && r2.push("'" + this.terminals_[f] + "'");
          S.showPosition ? _ = "Parse error on line " + (L + 1) + `:
` + S.showPosition() + `
Expecting ` + r2.join(", ") + ", got '" + (this.terminals_[w] || w) + "'" : _ = "Parse error on line " + (L + 1) + ": Unexpected " + (w == C ? "end of input" : "'" + (this.terminals_[w] || w) + "'"), this.parseError(_, {
            text: S.match,
        ...
        for (var b = this._currentRules(), d = 0; d < b.length; d++)
          if (c = this._input.match(this.rules[b[d]]), c && (!o || c[0].length > o[0].length)) {
            if (o = c, k = d, this.options.backtrack_lexer) {
              if (i = this.test_match(c, b[d]), i !== false)
                return i;
              if (this._backtrack) {
                o = false;
                continue;
...
var It = (t) => (t = Rn.sanitizeText(t, Jn()), B[t] || (B[t] = new Pt(t), Q.push(B[t])), B[t]);
var Ot = () => Q;
var $t = () => q;
var jt = () => ({
...
function Gt() {
  ...
  function E({ nodes: e, links: f }) {
    for (const [h2, r2] of e.entries())
      r2.index = h2, r2.sourceLinks = [], r2.targetLinks = [];
    const l = new Map(e.map((h2, r2) => [m(h2, r2, e), h2]));
    for (const [h2, r2] of f.entries()) {
      r2.index = h2;
      let { source: _, target: v } = r2;
      typeof _ != "object" && (_ = r2.source = ct(l, _)), typeof v != "object" && (v = r2.target = ct(l, v)), _.sourceLinks.push(r2), v.targetLinks.push(r2);
    }
    if (c != null)
      for (const { sourceLinks: h2, targetLinks: r2 } of e)
        h2.sort(c), r2.sort(c);
  }
  function L({ nodes: e }) {
    for (const f of e)
  ...
  function A({ nodes: e }) {
    const f = e.length;
    let l = new Set(e), h2 = /* @__PURE__ */ new Set(), r2 = 0;
    for (; l.size; ) {
      for (const _ of l) {
        _.depth = r2;
  ...
  function C({ nodes: e }) {
    const f = ot(e, (r2) => r2.depth) + 1, l = (s - t - u) / (f - 1), h2 = new Array(f);
    for (const r2 of e) {
      const _ = Math.max(0, Math.min(f - 1, Math.floor(i.call(null, r2, f))));
      r2.layer = _, r2.x0 = t + _ * l, r2.x1 = r2.x0 + u, h2[_] ? h2[_].push(r2) : h2[_] = [r2];
    }
    if (o)
      for (const r2 of h2)
        r2.sort(o);
  ...
  function j(e) {
    ...
    for (const l of e) {
      let h2 = n;
      for (const r2 of l) {
        r2.y0 = h2, r2.y1 = h2 + r2.value * f, h2 = r2.y1 + p;
        for (const _ of r2.sourceLinks)
          _.width = _.value * f;
      }
      h2 = (a - h2 + p) / (l.length + 1);
      for (let r2 = 0; r2 < l.length; ++r2) {
        const _ = l[r2];
        _.y0 += h2 * (r2 + 1), _.y1 += h2 * (r2 + 1);
      }
      O(l);
    }
  }
  function S(e) {
    ...
    for (let l = 0; l < d; ++l) {
      const h2 = Math.pow(0.99, l), r2 = Math.max(1 - h2, (l + 1) / d);
      $(f, h2, r2), M(f, h2, r2);
    }
  }
  function M(e, f, l) {
    for (let h2 = 1, r2 = e.length; h2 < r2; ++h2) {
      const _ = e[h2];
      for (const v of _) {
        let R = 0, z = 0;
  ...
  function $(e, f, l) {
    for (let h2 = e.length, r2 = h2 - 2; r2 >= 0; --r2) {
      const _ = e[r2];
      for (const v of _) {
        let R = 0, z = 0;
  ...
  function I(e, f, l, h2) {
    for (; l < e.length; ++l) {
      const r2 = e[l], _ = (f - r2.y0) * h2;
      _ > 1e-6 && (r2.y0 += _, r2.y1 += _), f = r2.y1 + p;
    }
  }
  function g(e, f, l, h2) {
    for (; l >= 0; --l) {
      const r2 = e[l], _ = (r2.y1 - f) * h2;
      _ > 1e-6 && (r2.y0 -= _, r2.y1 -= _), f = r2.y0 - p;
    }
  }
  ...
  function T(e, f) {
    ...
    for (const { target: h2, width: r2 } of e.sourceLinks) {
      if (h2 === f)
        break;
      l += r2 + p;
    }
    for (const { source: h2, width: r2 } of f.targetLinks) {
      if (h2 === e)
        break;
      l -= r2;
    }
...
${x}${Math.round(w * 100) / 100}${E}` : g;
  ...
  $.append("path").attr("d", ie()).attr("stroke", I).attr("stroke-width", (g) => Math.max(1, g.width)), A_(void 0, o, 0, b);
};
var oe = {
  draw: re
};
var le = (t) => t.replaceAll(/^[^\S\n\r]+|[^\S\n\r]+$/g, "").replaceAll(/([\n\r])+/g, `
`).trim();
var ae = H.parse.bind(H);
H.parse = (t) => ae(le(t));
var ye = {
  parser: H,
  db: zt,
...
//# sourceMappingURL=sankeyDiagram-3e8fe5d5-5VWTQB4N.js.map
