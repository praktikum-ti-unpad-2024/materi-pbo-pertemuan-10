...
// node_modules/mermaid/dist/timeline-definition-01cbd2c6.js
var K = function() {
  var n = function(f, i, r, h) {
    ...
    parse: function(i) {
      ...
      for (var k, L2, E2, Z, z = {}, B2, $, Y, O; ; ) {
        if (L2 = h[h.length - 1], this.defaultActions[L2] ? E2 = this.defaultActions[L2] : ((k === null || typeof k > "u") && (k = C()), E2 = w[L2] && w[L2][k]), typeof E2 > "u" || !E2.length || !E2[0]) {
          var J = "";
          O = [];
          for (B2 in w[L2])
            this.terminals_[B2] && B2 > V && O.push("'" + this.terminals_[B2] + "'");
...
Expecting ` + O.join(", ") + ", got '" + (this.terminals_[k] || k) + "'" : J = "Parse error on line " + (N + 1) + ": Unexpected " + (k == U ? "end of input" : "'" + (this.terminals_[k] || k) + "'"), this.parseError(J, {
        ...
        switch (E2[0]) {
          case 1:
            h.push(k), p.push(b.yytext), l.push(b.yylloc), h.push(E2[1]), k = null, A = b.yyleng, x = b.yytext, N = b.yylineno, I = b.yylloc;
            break;
          case 2:
            if ($ = this.productions_[E2[1]][1], z.$ = p[p.length - $], z._$ = {
              ...
              return Z;
            $ && (h = h.slice(0, -1 * $ * 2), p = p.slice(0, -1 * $), l = l.slice(0, -1 * $)), h.push(this.productions_[E2[1]][0]), p.push(z.$), l.push(z._$), Y = w[h[h.length - 2]][h[h.length - 1]], h.push(Y);
            break;
          case 3:
            return true;
        }
...
var it = () => q_;
var rt = function() {
  Q.length = 0, G.length = 0, R = "", F.length = 0, O_();
};
var at = function(n) {
  R = n, Q.push(n);
};
...
var Tt2 = function(n, t) {
  ...
  function c(g) {
    const u = vn().startAngle(Math.PI / 2).endAngle(3 * (Math.PI / 2)).innerRadius(7.5).outerRadius(6.8181818181818175);
    g.append("path").attr("class", "mouth").attr("d", u).attr("transform", "translate(" + t.cx + "," + (t.cy + 2) + ")");
  }
  function o(g) {
    const u = vn().startAngle(3 * Math.PI / 2).endAngle(5 * (Math.PI / 2)).innerRadius(7.5).outerRadius(6.8181818181818175);
...
var Mt = function(n, t, e) {
  ...
  et++;
  const c = 300 + 5 * 30;
  s.append("line").attr("id", "task" + et).attr("x1", a).attr("y1", t.y).attr("x2", a).attr("y2", c).attr("class", "task-line").attr("stroke-width", "1px").attr("stroke-dasharray", "4 2").attr("stroke", "#666"), Tt2(s, {
    cx: a,
    cy: 300 + (5 - t.score) * 30,
    score: t.score
  });
...
function yt(n, t) {
  n.each(function() {
    var e = Tt(this), a = e.text().split(/(\s+|<br>)/).reverse(), s, c = [], o = 1.1, y = e.attr("y"), g = parseFloat(e.attr("dy")), u = e.text(null).append("tspan").attr("x", 0).attr("y", y).attr("dy", g + "em");
...
var Rt = function(n, t, e, a) {
  var W, b;
  const s = Jn(), c = s.leftMargin ?? 50;
  L.debug("timeline", a.db);
  const o = s.securityLevel;
  let y;
  o === "sandbox" && (y = Tt("#i" + t));
  const u = (o === "sandbox" ? Tt(y.nodes()[0].contentDocument.body) : Tt("body")).select("#" + t);
  u.append("g");
  ...
  for (const [S, _] of m.entries()) {
    ...
    for (let k = 0; k < _.events.length; k++) {
      ...
      C += H.getVirtualNodeHeight(u, E2, s);
    }
    A = Math.max(A, C);
  }
  ...
  L.debug("bounds", V), v && u.append("text").text(v).attr("x", V.width / 2 - c).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), h = x ? i + r + 150 : r + 100, u.append("g").attr("class", "lineWrapper").append("line").attr("x1", c).attr("y1", h).attr("x2", V.width + 3 * c).attr("y2", h).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)"), A_(
    void 0,
    u,
...
var Wt = (n) => {
  ...
  for (let e = 0; e < n.THEME_COLOR_LIMIT; e++)
    n["lineColor" + e] = n["lineColor" + e] || n["cScaleInv" + e], Ci(n["lineColor" + e]) ? n["lineColor" + e] = B(n["lineColor" + e], 20) : n["lineColor" + e] = E(n["lineColor" + e], 20);
  for (let e = 0; e < n.THEME_COLOR_LIMIT; e++) {
    const a = "" + (17 - 3 * e);
    t += `
...
//# sourceMappingURL=timeline-definition-01cbd2c6-MKCXJN6C.js.map
