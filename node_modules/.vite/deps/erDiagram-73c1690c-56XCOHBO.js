...
// node_modules/mermaid/dist/erDiagram-73c1690c.js
var at = function() {
...
Expecting ` + q.join(", ") + ", got '" + (this.terminals_[w] || w) + "'" : et = "Parse error on line " + (Q + 1) + ": Unexpected " + (w == ot ? "end of input" : "'" + (this.terminals_[w] || w) + "'"), this.parseError(et, {
        ...
        switch (D[0]) {
          case 1:
            ...
            break;
          case 2:
            if (P2 = this.productions_[D[1]][1], z.$ = E[E.length - P2], z._$ = {
              ...
              return tt;
            P2 && (c = c.slice(0, -1 * P2 * 2), E = E.slice(0, -1 * P2), i = i.slice(0, -1 * P2)), c.push(this.productions_[D[1]][0]), E.push(z.$), i.push(z._$), lt = K[c[c.length - 2]][c[c.length - 1]], c.push(lt);
            break;
          case 3:
            return true;
        }
...
var dt = function(t, e = void 0) {
  return U[t] === void 0 ? (U[t] = { attributes: [], alias: e }, L.info("Added new entity :", t)) : U[t] && !U[t].alias && e && (U[t].alias = e, L.info(`Add alias '${e}' to entity '${t}'`)), U[t];
};
var vt = () => U;
var Bt = function(t, e) {
  let r = dt(t), u;
  for (u = e.length - 1; u >= 0; u--)
    r.attributes.push(e[u]), L.debug("Added attribute ", e[u].attributeName);
};
var Ct = function(t, e, r, u) {
  let l = {
    entityA: t,
    roleA: e,
    entityB: r,
...
var ee = (t, e, r) => {
  ...
  r.forEach((T) => {
    ...
    let R = 0;
    const O = v1(T.attributeType), L2 = t.append("text").classed("er entityLabel", true).attr("id", `${M}-type`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Jn().fontFamily).style("font-size", p + "px").text(O), S = t.append("text").classed("er entityLabel", true).attr("id", `${M}-name`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Jn().fontFamily).style("font-size", p + "px").text(T.attributeName), a = {};
    a.tn = L2, a.nn = S;
    const n = L2.node().getBBox(), c = S.node().getBBox();
    if (m = Math.max(m, n.width), g = Math.max(g, c.width), R = Math.max(n.height, c.height), h) {
      const d = T.attributeKeyTypeList !== void 0 ? T.attributeKeyTypeList.join(",") : "", E = t.append("text").classed("er entityLabel", true).attr("id", `${M}-key`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Jn().fontFamily).style("font-size", p + "px").text(d);
      a.kn = E;
      const i = E.node().getBBox();
      x = Math.max(x, i.width), R = Math.max(R, i.height);
    }
    if (_) {
      const d = t.append("text").classed("er entityLabel", true).attr("id", `${M}-comment`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Jn().fontFamily).style("font-size", p + "px").text(T.attributeComment || "");
      a.cn = d;
  ...
  if (r.length > 0) {
    ...
    o.forEach((O) => {
      ...
      const S = t.insert("rect", "#" + O.tn.node().id).classed(`er ${R}`, true).attr("x", 0).attr("y", M).attr("width", m + l * 2 + T).attr("height", O.height + u * 2), a = parseFloat(S.attr("x")) + parseFloat(S.attr("width"));
      O.nn.attr(
        "transform",
        "translate(" + (a + l) + "," + L2 + ")"
      );
...
var re = function(t, e, r) {
  ...
  return u.forEach(function(p) {
    ...
    l = l === void 0 ? f : l;
    const h = "text-" + f, _ = o.append("text").classed("er entityLabel", true).attr("id", h).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "middle").style("font-family", Jn().fontFamily).style("font-size", k.fontSize + "px").text(e[p].alias ?? p), { width: m, height: g } = ee(
      o,
      _,
      e[p].attributes
...
var ne = function(t, e, r, u, l) {
  ...
  const p = r.edge(
    ...
    return N.y;
  }).curve(jf), o = t.insert("path", "#" + u).classed("er relationshipLine", true).attr("d", f(p.points)).style("stroke", k.stroke).style("fill", "none");
  ...
  const _ = o.node().getTotalLength(), m = o.node().getPointAtLength(_ * 0.5), g = "rel" + ct, y = t.append("text").classed("er relationshipLabel", true).attr("id", g).attr("x", m.x).attr("y", m.y).style("text-anchor", "middle").style("dominant-baseline", "middle").style("font-family", Jn().fontFamily).style("font-size", k.fontSize + "px").text(e.roleA).node().getBBox();
...
var se = function(t, e, r, u) {
  k = Jn().er, L.info("Drawing ER diagram");
  const l = Jn().securityLevel;
  let p;
  l === "sandbox" && (p = Tt("#i" + e));
  const o = (l === "sandbox" ? Tt(p.nodes()[0].contentDocument.body) : Tt("body")).select(`[id='${e}']`);
  B.insertMarkers(o, k);
  let h;
  h = new Ze({
    multigraph: true,
  ...
  Yt(h), ie(o, h), m.forEach(function(I) {
    ne(o, I, h, _, u);
  });
  const g = k.diagramPadding;
  oi.insertTitle(o, "entityTitleText", k.titleTopMargin, u.db.getDiagramTitle());
  const x = o.node().getBBox(), y = x.width + g * 2, N = x.height + g * 2;
  Pl(o, N, y, k.useMaxWidth), o.attr("viewBox", `${x.x - g} ${x.y - g} ${y} ${N}`);
};
...
//# sourceMappingURL=erDiagram-73c1690c-56XCOHBO.js.map
