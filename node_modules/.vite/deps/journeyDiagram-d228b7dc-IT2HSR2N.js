...
// node_modules/mermaid/dist/journeyDiagram-d228b7dc.js
var G = function() {
  var t = function(p, s, r, a) {
    for (r = r || {}, a = p.length; a--; r[p[a]] = s)
      ;
    return r;
  }, e = [6, 8, 10, 11, 12, 14, 16, 17, 18], i = [1, 9], o = [1, 10], n = [1, 11], h = [1, 12], c = [1, 13], d2 = [1, 14], y = {
    trace: function() {
    },
    yy: {},
    ...
    performAction: function(s, r, a, u, f, l2, w) {
      var k = l2.length - 1;
      switch (f) {
        case 1:
          return l2[k - 1];
        ...
        case 7:
          ...
          break;
        case 8:
          u.setDiagramTitle(l2[k].substr(6)), this.$ = l2[k].substr(6);
          break;
        case 9:
          this.$ = l2[k].trim(), u.setAccTitle(this.$);
          break;
        case 10:
        case 11:
          this.$ = l2[k].trim(), u.setAccDescription(this.$);
          break;
        case 12:
          u.addSection(l2[k].substr(8)), this.$ = l2[k].substr(8);
          break;
        case 13:
          u.addTask(l2[k - 1], l2[k]), this.$ = "task";
          break;
      }
    },
    table: [{ 3: 1, 4: [1, 2] }, { 1: [3] }, t(e, [2, 2], { 5: 3 }), { 6: [1, 4], 7: 5, 8: [1, 6], 9: 7, 10: [1, 8], 11: i, 12: o, 14: n, 16: h, 17: c, 18: d2 }, t(e, [2, 7], { 1: [2, 1] }), t(e, [2, 3]), { 9: 15, 11: i, 12: o, 14: n, 16: h, 17: c, 18: d2 }, t(e, [2, 5]), t(e, [2, 6]), t(e, [2, 8]), { 13: [1, 16] }, { 15: [1, 17] }, t(e, [2, 11]), t(e, [2, 12]), { 19: [1, 18] }, t(e, [2, 4]), t(e, [2, 9]), t(e, [2, 10]), t(e, [2, 13])],
    defaultActions: {},
    ...
    parse: function(s) {
      var r = this, a = [0], u = [], f = [null], l2 = [], w = this.table, k = "", R = 0, Z = 0, ot = 2, J = 1, ct = l2.slice.call(arguments, 1), x2 = Object.create(this.lexer), S = { yy: {} };
...
Expecting ` + B.join(", ") + ", got '" + (this.terminals_[_] || _) + "'" : q = "Parse error on line " + (R + 1) + ": Unexpected " + (_ == J ? "end of input" : "'" + (this.terminals_[_] || _) + "'"), this.parseError(q, {
        ...
        switch (b[0]) {
          case 1:
            a.push(_), f.push(x2.yytext), l2.push(x2.yylloc), a.push(b[1]), _ = null, Z = x2.yyleng, k = x2.yytext, R = x2.yylineno, Y = x2.yylloc;
            break;
          case 2:
            if (T = this.productions_[b[1]][1], I.$ = f[f.length - T], I._$ = {
              first_line: l2[l2.length - (T || 1)].first_line,
              last_line: l2[l2.length - 1].last_line,
              first_column: l2[l2.length - (T || 1)].first_column,
              last_column: l2[l2.length - 1].last_column
            }, ht && (I._$.range = [
              l2[l2.length - (T || 1)].range[0],
              ...
              return O;
            T && (a = a.slice(0, -1 * T * 2), f = f.slice(0, -1 * T), l2 = l2.slice(0, -1 * T)), a.push(this.productions_[b[1]][0]), f.push(I.$), l2.push(I._$), K = w[a[a.length - 2]][a[a.length - 1]], a.push(K);
            break;
          case 3:
            return true;
        }
...
` + r + "^";
      ...
      next: function() {
        ...
        for (var f = this._currentRules(), l2 = 0; l2 < f.length; l2++)
          if (a = this._input.match(this.rules[f[l2]]), a && (!r || a[0].length > r[0].length)) {
            if (r = a, u = l2, this.options.backtrack_lexer) {
              if (s = this.test_match(a, f[l2]), s !== false)
                return s;
              if (this._backtrack) {
                r = false;
                continue;
...
var F = [];
var Tt2 = function() {
  H.length = 0, V.length = 0, A = "", F.length = 0, O_();
};
var $t = function(t) {
  A = t, H.push(t);
};
...
var Ft = function(t, e) {
  ...
  return e.score > 3 ? h(n) : e.score < 3 ? c(n) : d2(n), o;
};
var rt = function(t, e) {
...
var Lt = function(t, e) {
  function i(n, h, c, d2, y) {
    return n + "," + h + " " + (n + c) + "," + h + " " + (n + c) + "," + (h + d2 - y) + " " + (n + c - y * 1.2) + "," + (h + d2) + " " + n + "," + (h + d2);
  }
...
var lt = function() {
  function t(n, h, c, d2, y, m, g2, p) {
    const s = h.append("text").attr("x", c + y / 2).attr("y", d2 + m / 2 + 5).style("font-color", p).style("text-anchor", "middle").text(n);
    o(s, g2);
  }
  function e(n, h, c, d2, y, m, g2, p, s) {
    const { taskFontSize: r, taskFontFamily: a } = p, u = n.split(/<br\s*\/?>/gi);
    for (let f = 0; f < u.length; f++) {
      const l2 = f * r - r * (u.length - 1) / 2, w = h.append("text").attr("x", c + y / 2).attr("y", d2).attr("fill", s).style("text-anchor", "middle").style("font-size", r).style("font-family", a);
      w.append("tspan").attr("x", c + y / 2).attr("dy", l2).text(u[f]), w.attr("y", d2 + m / 2).attr("dominant-baseline", "central").attr("alignment-baseline", "central"), o(w, g2);
    }
  }
  function i(n, h, c, d2, y, m, g2, p) {
    const s = h.append("switch"), a = s.append("foreignObject").attr("x", c).attr("y", d2).attr("width", y).attr("height", m).attr("position", "fixed").append("xhtml:div").style("display", "table").style("height", "100%").style("width", "100%");
    a.append("div").attr("class", "label").style("display", "table-cell").style("text-align", "center").style("vertical-align", "middle").text(n), e(n, s, c, d2, y, m, g2, p), o(a, g2);
  }
  function o(n, h) {
    for (const c in h)
      c in h && n.attr(c, h[c]);
...
var Ot = function(t, e, i, o) {
  const n = Jn().journey, h = Jn().securityLevel;
  let c;
  h === "sandbox" && (c = Tt("#i" + e));
  const d2 = h === "sandbox" ? Tt(c.nodes()[0].contentDocument.body) : Tt("body");
  v.init();
  const y = d2.select("#" + e);
  L.initGraphics(y);
  ...
  Pl(y, a, u, n.useMaxWidth), y.append("line").attr("x1", P).attr("y1", n.height * 4).attr("x2", u - P - 4).attr("y2", n.height * 4).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)");
  const f = g2 ? 70 : 0;
...
var v = {
  ...
  insert: function(t, e, i, o) {
    const n = Math.min(t, i), h = Math.max(t, i), c = Math.min(e, o), d2 = Math.max(e, o);
    this.updateVal(v.data, "startx", n, Math.min), this.updateVal(v.data, "starty", c, Math.min), this.updateVal(v.data, "stopx", h, Math.max), this.updateVal(v.data, "stopy", d2, Math.max), this.updateBounds(n, c, h, d2);
  },
...
var qt = function(t, e, i) {
  ...
  for (const [p, s] of e.entries()) {
    if (n !== s.section) {
      ...
      let a = 0;
      const u = s.section;
      for (let l2 = p; l2 < e.length && e[l2].section == u; l2++)
        a = a + 1;
      const f = {
    ...
    s.x = p * o.taskMargin + p * o.width + P, s.y = c, s.width = o.diagramMarginX, s.height = o.diagramMarginY, s.colour = m, s.fill = y, s.num = g2, s.actors = r, L.drawTask(t, s, o), v.insert(s.x, s.y, s.x + s.width + o.taskMargin, 300 + 5 * 30);
  }
};
var nt = {
  setConf: zt,
...
//# sourceMappingURL=journeyDiagram-d228b7dc-IT2HSR2N.js.map
